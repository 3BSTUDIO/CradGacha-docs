...
- **`/gacha setup` รันอัตโนมัติตอนเปิดเซิร์ฟครั้งแรก** เมื่อเซิร์ฟบูตเสร็จ — ติดตั้งใหม่ได้เมนูพร้อมใช้ทันทีไม่ต้องทำมือ
- **`/gacha setup` ติดตั้ง blueprint ของ ModelEngine** (`cradgacha_open`) เข้า `ModelEngine/blueprints/` + reload แล้ว merge pack โมเดลให้ — โมเดล 3D ใช้ได้เลย ไม่รู้สึกว่า pack ไม่ครบ

**เมนู / cursor**

- **ความไวเมาส์เหลือค่าเดียว** (เดิมแยก X/Y ทำให้สับสนและดูเหมือนพื้นที่แคบลง) — ระยะที่เคอร์เซอร์เอื้อมถึงกำหนดด้วยขนาดเมนู ไม่ใช่ความเร็ว
- **แจ้ง "ไอเทมไม่พอ" บอกชื่อไอเทม** ที่ขาด (เช่น *Not enough Diamond!*)
- **ประสิทธิภาพ:** loop cursor ต่อ tick ไม่อ่าน config ซ้ำทุก tick แล้ว (snapshot + refresh ตอน reload) — เบาลงเมื่อมีผู้เล่นเปิดเมนูพร้อมกันเยอะ

**พรีเมียม — เครื่องมือแก้ไข**

- Reward editor: ปุ่มเลือกไอเทมเปลี่ยนชื่อเป็น **[ Inventory ]** และเพิ่มปุ่ม **[ Reset ]** ยกเลิกการแก้ไขที่ยังไม่บันทึก (ยืนยัน 2 คลิก)
